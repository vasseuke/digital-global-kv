 ...
 * Base block: cards
 * Source: https://www.dunkincreamer.com/coffee-creamer/coffee-creamer/ (section.discoverflavors)
 ...
 *                  > .find-des > .smalltext span   (size label, e.g. "32oz & 48oz")
 *                             > .navlabel          (product name; may contain sup/newtag)
 *     .btn-wrapper > a (SHOP NOW)
 *
 * Variations handled:
 *   - `.navlabel` may include a "newtag" badge and sup/sub markup — preserved by
 *     promoting its content into a heading.
 *   - SHOP NOW link points at javascript:void — kept as authored (link text is
 *     the meaningful content); rewritten to the card's product URL when available.
...
export default function parse(element, { document }) {
  ...
  cards.forEach((li) => {
    ...
    // SHOP NOW CTA. Point the link at the card's product page if the button
    // href is a non-navigational javascript: stub.
    ...
    if (shop) {
      ...
      // Rewrite non-navigational hrefs (javascript: stubs — note source typo
      // "javscript:" — empty, or "#") to the card's product page.
